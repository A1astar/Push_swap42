...
/*   Updated: 2025/01/09 18:21:31 by alacroix         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	push_head_first(t_node **a_head)
{
	...
	if (clockwise_target_search(a_head, &min) < reverse_target_search(a_head,
			&min))
		while (*a_head != min)
			rotate(a_head, 'a');
	else
...
static char	*inst_set_of(t_node **current, t_node **src_head, t_node **dst_head)
{
	...
	target_pos_inst(&tab, current, dst_head);
...
static char	*find_cheapest_inst_set(t_node **src_head, t_node **dst_head)
{
	...
	while (1)
	{
		current_set = inst_set_of(&current, src_head, dst_head);
		if (!current_set)
